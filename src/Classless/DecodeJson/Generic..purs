...
sumOf :: forall a spec r. Rep.Generic a r => DecodeRep spec r => { | spec } -> Json -> Either JsonDecodeError a
sumOf spec = sumOfWith spec defaultEncoding

-- | Decode `Json` representation of a value which has a `Generic` type.
-- | Takes a record for encoding settings.
sumOfWith :: forall a spec r. Rep.Generic a r => DecodeRep spec r => { | spec } -> Encoding -> Json -> Either JsonDecodeError a
sumOfWith spec e = map Rep.to <<< decodeRepWith spec e
