...
class SumOf sumSpec a | a -> sumSpec where
  sumOf :: { | sumSpec } -> (a -> Json)

instance (Generic a rep, EncodeRep sumSpec rep) => SumOf sumSpec a where
  sumOf spec = sumOfWith spec defaultEncoding

-- | Encode any `Generic` data structure into `Json`.
-- | Takes a record for encoding settings.
sumOfWith :: forall sumSpec a r. Rep.Generic a r => EncodeRep sumSpec r => { | sumSpec } -> Encoding -> a -> Json
sumOfWith spec e = encodeRepWith spec e <<< Rep.from
